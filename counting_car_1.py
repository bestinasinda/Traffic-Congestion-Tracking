def video_analysis(route_name):
    ...
    class Sensor:
        def __init__(self, kordinat1, kordinat2, frame_weight, frame_lenght):
            ...
            self.mask = np.zeros((frame_weight, frame_lenght, 1),
                                 np.uint8)*abs(self.kordinat2.y-self.kordinat1.y)
    ...
    while datetime.datetime.now() < run_time:
        ...
        opening_image = cv2.morphologyEx(
            deleted_background, cv2.MORPH_OPEN, kernel)
        ret, opening_image = cv2.threshold(
            opening_image, 125, 255, cv2.THRESH_BINARY)
       ...
       #  _, cnts,_=cv2.findContours(opening_image,cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
        cnts, hierachy = cv2.findContours(
            opening_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        result = cropped_image.copy()

        zeros_image = np.zeros(
            (cropped_image.shape[0], cropped_image.shape[1], 1), np.uint8)
        ...
        for cnt in cnts:
            ...
            if (w > 75 and h > 70 and w < 150 and h < 160):
                cv2.rectangle(result, (x, y), (x+w, y+h),
                              (255, 0, 0), thickness=2)
                cv2.rectangle(zeros_image, (x, y), (x+w, y+h),
                              (255), thickness=cv2.FILLED)

        # detect whether there is car via bitwise_and
        mask1 = np.zeros(
            (zeros_image.shape[0], zeros_image.shape[1], 1), np.uint8)
        mask_result = cv2.bitwise_or(
            zeros_image, zeros_image, mask=Sensor1.mask)
